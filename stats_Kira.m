    ...
    for iSubj = 1:length(S.subjects)
        for iSess = 1:length(S.sessions)
            ...
            for iAnal = 1:length(Analyses)
                ...
                [raw,tnorm,results_struct] = time_norm_per_session(data_struct,results_struct);  % add all participants to the results struct and time normalise data
...
for iLeg = 1:2
    ...
    muscle_names_short = {};                                                                                        % remove '_l' or '_r' and numbers from the muscle names
    for iMusc = 1:length(muscle_names)
        muscle_names_short{end+1,1} = muscle_names{iMusc}(1:end-2);
        if any(contains(muscle_names_short{end}(end),{'1','2','3'}))
