...
if run_stats
    ...
    %         end
    %     end
    Results = create_muscle_groups(Results); % add muscle groups
    ...
    save('Results_BW.mat', 'Results_BW')
        ...
        if plot_muscles
            for o = size(angles,1)
                ...
                for j=1:size(muscles_r,1)
                        ...
                        axes(ax(count)); hold on; grid on;
                        title(muscle_groups{j},'Interpreter', 'None')
%                         ylim([0 2])

                        %                             plot(Results_BW.JRL.(sessions{iDOF}).(legs{j}).(joints{k})); hold on;
                        scatter(bone_geom.(angles{k}) (1:10), [Results_BW.SO.session1.(legs{1}).(['peak_' muscles_l{j} '_val'])(1,:), Results_BW.SO.session1.(legs{2}).(['peak_' muscles_r{j} '_val'])(1,:)],36,dot_colors(1,:),'filled')
                        scatter(bone_geom.(angles{k}) (11:20), [Results_BW.SO.session2.(legs{1}).(['peak_' muscles_l{j} '_val'])(1,:), Results_BW.SO.session2.(legs{2}).(['peak_' muscles_r{j} '_val'])(1,:)],36,dot_colors(2,:),'filled')
                        scatter(bone_geom.(angles{k}) (21:end), [Results_BW.SO.session3.(legs{1}).(['peak_' muscles_l{j} '_val'])(1,:), Results_BW.SO.session3.(legs{2}).(['peak_' muscles_r{j} '_val'])(1,:)],36,dot_colors(end,:),'filled')
...
%                     lg.Position = [0.8 0.12 0.05 0.1]
            ...
            for j=1:size(joints,1)
                nb_plots = size(angles,1)*size(seg,1);
                [ax, pos,FirstCol,LastRow,LastCol] = tight_subplot(nb_plots,0,[],[0.1 0.05],[0.1 0.05]);
%                 title(joints{j})
                count = 1;
                for k=1:size(angles,1)
                    for l=1:size(seg,1)
                    axes(ax(count)); hold on; grid on;
                    title([joints{j} ' ' angles{k} ' ' seg{l}])

                    %                             plot(Results_BW.JRL.(sessions{iDOF}).(legs{j}).(joints{k})); hold on;
                    scatter(bone_geom.(angles{k}) (1:10), [Results_BW.JRL.session1.(legs{1}).(['peak_' joints{j} seg{l} '_val'])(1,:), Results_BW.JRL.session1.(legs{2}).(['peak_' joints{j} seg{l} '_val'])(1,:)],36,dot_colors(1,:),'filled')
                    scatter(bone_geom.(angles{k}) (11:20), [Results_BW.JRL.session2.(legs{1}).(['peak_' joints{j} seg{l} '_val'])(1,:), Results_BW.JRL.session2.(legs{2}).(['peak_' joints{j} seg{l} '_val'])(1,:)],36,dot_colors(2,:),'filled')
                    scatter(bone_geom.(angles{k}) (21:end), [Results_BW.JRL.session3.(legs{1}).(['peak_' joints{j} seg{l} '_val'])(1,:), Results_BW.JRL.session3.(legs{2}).(['peak_' joints{j} seg{l} '_val'])(1,:)],36,dot_colors(end,:),'filled')
...
%         Results.JRL.(session).(leg).peak_ACF_loc(2,:) = loc2;
        ...
        %         Results.JRL.(ses1sion).(leg).peak_KCF_loc(1,:) = Results.JRL.(session).(leg).peak_KCF_loc(1,:) + lim1-1;
        %         Results.JRL.(session).(leg).peak_KCF_loc(2,:) = Results.JRL.(session).(leg).peak_KCF_loc(2,:) + lim2-1;

        % find peak component JCF
        % HCF
        [Results.JRL.(session).(leg).peak_HCF_fx_val,~] = calc_max(Results.JRL.(session).(leg).(['hip_' leg(1) '_on_pelvis_in_pelvis_fx']));
        [Results.JRL.(session).(leg).peak_HCF_fy_val,~] = calc_max(Results.JRL.(session).(leg).(['hip_' leg(1) '_on_pelvis_in_pelvis_fy']));
        [Results.JRL.(session).(leg).peak_HCF_fz_val,~] = calc_max(Results.JRL.(session).(leg).(['hip_' leg(1) '_on_pelvis_in_pelvis_fz']));

        % KCF
        [Results.JRL.(session).(leg).peak_KCF_fx_val,~] = calc_max(Results.JRL.(session).(leg).(['knee_' leg(1) '_on_tibia_' leg(1) '_in_tibia_' leg(1) '_fx']));
        [Results.JRL.(session).(leg).peak_KCF_fy_val,~] = calc_max(Results.JRL.(session).(leg).(['knee_' leg(1) '_on_tibia_' leg(1) '_in_tibia_' leg(1) '_fy']));
        [Results.JRL.(session).(leg).peak_KCF_fz_val,~] = calc_max(Results.JRL.(session).(leg).(['knee_' leg(1) '_on_tibia_' leg(1) '_in_tibia_' leg(1) '_fz']));

        % ACF
        [Results.JRL.(session).(leg).peak_ACF_fx_val,~] = calc_max(Results.JRL.(session).(leg).(['ankle_' leg(1) '_on_talus_' leg(1) '_in_talus_' leg(1) '_fx']));
        [Results.JRL.(session).(leg).peak_ACF_fy_val,~] = calc_max(Results.JRL.(session).(leg).(['ankle_' leg(1) '_on_talus_' leg(1) '_in_talus_' leg(1) '_fy']));
        [Results.JRL.(session).(leg).peak_ACF_fz_val,~] = calc_max(Results.JRL.(session).(leg).(['ankle_' leg(1) '_on_talus_' leg(1) '_in_talus_' leg(1) '_fz']));
