function [output] = c3dExport_f(c3dpath,filename)
% ----------------------------------------------------------------------- %
...
%% Get the path to a C3D file (will be done in main file)
% [filename, path] = uigetfile('*.c3d'); ----------------- changed ---------------
...
filename = erase(filename, '.c3d');   %------------------ changed --------------------
c3d.writeTRC(append(filename,'.trc')); %---------------- changed---------------------
...
c3d.writeMOT(append(filename, '.mot')); %------------------ changed ---------------------
