function momentArmsAreWrong = checkMuscleMomentArms_l(modelFilename, motionFilename)
...
flexIndexLknee = model.getCoordinateSet().getIndex('knee_angle_l');
flexCoordLknee = model.updCoordinateSet().get(flexIndexLknee);
flexIndexLank = model.getCoordinateSet().getIndex('ankle_angle_l');
flexCoordLank = model.updCoordinateSet().get(flexIndexLank);
...
for i = 1:motion.getSize()
    ...
    flexAngleLknee = motion.getStateVector(i-1).getData().get(flexIndexLknee) / 180 * pi;
    flexAngleLank = motion.getStateVector(i-1).getData().get(flexIndexLank) / 180 * pi;
    ...
    for j = 1:length(muscleIndices)
        ...
        if strcmp(muscleNames{j}(end), 'l')
            ...
            flexMomentArm = model.getMuscles().get(muscleIndex).computeMomentArm(state, flexCoordLknee);
            flexMomentArms(i,j) = flexMomentArm;
            rotMomentArm = model.getMuscles().get(muscleIndex).computeMomentArm(state, flexCoordLank);
            rotMomentArms(i,j) = rotMomentArm;
        end
    end
end
